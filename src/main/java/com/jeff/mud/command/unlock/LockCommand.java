	...
	protected LockUnlockTemplate getLockUnlockTemplate() {
		return new LockUnlockTemplate() {
			...
			public void notContainsDirection(CommandDataCarrier input) {
				customMessagingTemplate.sendToYou(input.getUsername(), Template.defaultMessage, String.format("%s은(는) 잠글 수 없습니다.", input.getTarget()));
			}

			@Override
			public void matchedKey(CommandDataCarrier input, Door door, String direction) {
				door.lock();
				customMessagingTemplate.sendToYou(input.getUsername(), Template.defaultMessage, String.format("당신은 %s문을 잠궜습니다.", direction));
				customMessagingTemplate.sendToRoomWithOutMe(input, Template.defaultMessage, String.format("%s이(가) %s문을 잠궜습니다.", input.getPlayer().getName(), direction));
			}
			...
			public void notLockedOrUnlocked(CommandDataCarrier input, String direction) {
				customMessagingTemplate.sendToYou(input.getUsername(), Template.defaultMessage, String.format("%s문은 이미 잠겨있습니다.", direction));
			}
